...
export function logger(config: LoggerConfig = {}) {
  ...
  const log = new Elysia({
    ...
    .onAfterResponse(
      { as: "global" },
      async ({ store, request, path, set, responseValue }) => {
        const duration = formatDuration(process.hrtime.bigint() - store.requestStartTime);
        const method = formatMethod(request.method);
        const timestamp = formatTimestamp(new Date());
        const _path = formatPath(path);

        let status = formatStatusCode(set.status, 200);
        if (responseValue instanceof ElysiaCustomStatusResponse) {
          status = responseValue.code;
        }
    ...
    .get("/logger/success-set", ({ set }) => {
      set.status = 204;
      return "Success Set";
    })
    .get("/logger/success-status", ({ status }) => status(204, "Success Status"))

    .get("/logger/fail-set", ({ set }) => {
      set.status = 404;
      return "Fail Set";
    })
    .get("/logger/fail-status", ({ status }) => status(404, "Fail Status"))

    .get("/logger/error-set", ({ set }) => {
      set.status = 502;
      throw new Error("Error set");
    })
    .get("/logger/error-status", ({ status }) => {
      throw status(502, "Error Status");
    })

    .get("/logger/redirect-point", () => "Redirected!")
    .post("/logger/redirect-test", ({ redirect }) =>
      redirect("/logger/redirect-point", 302)
    );
